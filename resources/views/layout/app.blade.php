...
<script>
    ...
    function initCharge() {
        initSliderInput('chargeVMax', 'v', {'min': 12, 'max': 16.5, 'start': 14.8, 'step': 0.05, 'decimals': 2}, {'min': 12, 'max': 16.5, 'step': 0.05, 'decimals': 2});
        initSliderInput('chargeVPivot', 'v', {'min': 13.4, 'max': 14.7, 'start': 14.2, 'step': 0.05, 'decimals': 2}, {'min': 13.4, 'max': 14.7, 'step': 0.05, 'decimals': 2});
        initSliderInput('chargeAMax', 'a', {'min': 0.1, 'max': 1.8, 'start': 0.9, 'step': 0.05, 'decimals': 2}, {'min': 0.1, 'max': 1.8, 'step': 0.05, 'decimals': 2});

        initSliderInput('asymADischarge', 'a', {'min': 0.05, 'max': 0.7, 'start': 0.1, 'step': 0.05, 'decimals': 2}, {'min': 0.05, 'max': 0.7, 'step': 0.05, 'decimals': 2});
        initSliderInput('asymDur', 'seconds', {'min': 5, 'max': 25, 'start': 5}, {'min': 5, 'max': 25});
        initSliderInput('asymRatio', 'percent');

        initSliderInput('chargeEndParamA', 'a', {'min': 0.05, 'max': 0.2, 'start': 0.1, 'step': 0.05, 'decimals': 2}, {'min': 0.05, 'max': 0.2, 'step': 0.05, 'decimals': 2});
        ...
        $('#asymTrigger').change(function () {
            if (this.checked)
                $('#asymParamDiv').removeClass('d-none');
            else
                $('#asymParamDiv').addClass('d-none');
        });

        $('#chargeEndType').change(function () {
            if (this.checked) {
                $('#chargeEndTypeSpan').text('напряжению');
                $('#chargeEndParamA').addClass('d-none');
                $('#chargeEndParamA2').addClass('d-none');
                $('#chargeEndParamV').removeClass('d-none');
            }
            else {
                $('#chargeEndTypeSpan').text('току');
                $('#chargeEndParamV').addClass('d-none');
                $('#chargeEndParamA').removeClass('d-none');
                $('#chargeEndParamA2').removeClass('d-none');
            }
        });
    }
    ...
    function initExtra() {
        ...
        $inputV[0].TouchSpin({
            ...
            .change(function () {
                // $sliderV.noUiSlider.set(this.value);
                $sliderV.noUiSlider.setHandle(0, this.value, true);
            });
        ...
        $inputV[1].TouchSpin({
            ...
            .change(function () {
                // $sliderV.noUiSlider.set(this.value);
                $sliderV.noUiSlider.setHandle(1, this.value, true);
            });

        $('#extraChargeTrigger').change(function () {
            if (this.checked)
                $('#extraChargeParamDiv').removeClass('d-none');
            else
                $('#extraChargeParamDiv').addClass('d-none');
        });
    }

    function initPostCharge() {
        initSliderInput('postChargeStoreV', 'v', {'start': 13.6, 'min': 12.4, 'max': 14.8, 'step': 0.05, 'decimals': 2}, {'min': 12.4, 'max': 14.8, 'step': 0.05, 'decimals': 2});
        initSliderInput('postChargeStoreA', 'a', {'max': 0.2, 'start': 0.1}, {'max': 0.2});
        initSliderInput('postChargeRechargeV', 'v', {'start': 12.4, 'min': 11.9, 'max': 12.6, 'step': 0.05, 'decimals': 2}, {'min': 11.9, 'max': 12.6, 'step': 0.05, 'decimals': 2});

        $('#postChargeTrigger').change(function () {
            if (this.checked)
                $('#postChargeParamDiv').removeClass('d-none');
            else
                $('#postChargeParamDiv').addClass('d-none');
        });

        $('#postChargeType').change(function () {
            if (this.checked) {
                $('#postChargeTypeSpan').text('Повторный заряд');
                $('#postChargeParamStore').addClass('d-none');
                $('#postChargeParamRecharge').removeClass('d-none');
            }
            else {
                $('#postChargeTypeSpan').text('Поддержание заряда');
                $('#postChargeParamRecharge').addClass('d-none');
                $('#postChargeParamStore').removeClass('d-none');
            }
        });
    }
